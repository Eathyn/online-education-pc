...
// 动态读取 src 下的子目录作为 scope 选项
const scopes = fs.existsSync(srcDir)
  ? fs
...
const commitConfig: UserConfig = {
  ...
  prompt: {
    ...
    types: [
      ...
      // { value: "refactor", name: "重构:   ♻️  代码重构（不包括 bug 修复、功能新增）" },
      // { value: "perf", name: "性能:    ⚡️  性能优化" },
      // { value: "test", name: "测试:   ✅  添加疏漏测试或已有测试改动" },
      // { value: "build", name: "构建:   📦️  构建流程、外部依赖变更（如升级 npm 包、修改 webpack 配置等）" },
      // { value: "ci", name: "集成:   🎡  修改 CI 配置、脚本" },
      // { value: "revert", name: "回退:   ⏪️  回滚 commit" },
